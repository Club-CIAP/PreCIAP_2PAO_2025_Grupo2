# housing_price_prediction guide installation
...
conda activate housing_price_prediction
```
...
mamba activate housing_price_prediction
```
or 
...
To move beyond notebook prototyping, all reusable code should go into the `housing_price_prediction/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `housing_price_prediction` folder and use the modules inside your notebooks :
...
```
This will install the project module in "editable" mode so that any changes you make to the code in the housing_price_prediction/ folder will be reflected immediately without needing to reinstall.
...
```python
from housing_price_prediction.utils.paths import data_dir
data_dir()
```
